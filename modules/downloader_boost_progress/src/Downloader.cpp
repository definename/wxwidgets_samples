...
boost::signals2::connection Downloader::DoOnProgressSignal(const OnProgressSignal::slot_type& signal)
{
	return onProgress_.connect(signal);
}
...
void Downloader::HandleReadHeaders(const boost::system::error_code& err)
{
	try
	{
		...
		while (std::getline(response_stream, rawHeader) && rawHeader != "\r")
		{
	...
	try
	{
		...
		if (!err)
		{
			*stream_ << &response_;
			...
			//! Calculates progress.
			if (type_ == DownloadInstaller)
			{
				Headers::const_iterator it = headers_.find("Content-Length");
				if (it == headers_.end())
					BOOST_THROW_EXCEPTION(std::runtime_error("Invalide content length"));

				//! Increments progress once per one percent.
				std::streamsize percent = boost::lexical_cast<std::streamsize>(it->second) / 100;
				int actualPercentage = static_cast<int>(stream_->tellp() / percent);
				if (actualPercentage > percentage_)
				{
					percentage_ = actualPercentage;
					onProgress_();
				}
			}
