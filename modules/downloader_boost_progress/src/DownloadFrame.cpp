	...
	, panel_(new DownloadPanel(this))
{
	...
	downloader_->DoOnProgressSignal(boost::bind(&DownloadFrame::Post, this, DownloadingProgress, ""));
...
void DownloadFrame::CustomEventHandler(wxCommandEvent& e)
{
	try
	{
		if (e.GetId() == DownloadingCompleted)
		{
			...
			{
				panel_->DownloadedFileVersion(wxT("Installer file version has been downloaded."));
			...
			{
				panel_->DownloadedInstaller(wxT("Installer image has been downloaded."));
				Downloader::FileBuffer& stream = downloader_->GetBuffer();
		...
		else if (e.GetId() == DownloadingProgress)
		{
			panel_->Progress();
		}
...
void DownloadFrame::OnCloseWindow(wxCloseEvent& WXUNUSED(e))
{
	...
	//! Todo. Normally must be called Destroy() but it does not work in some cases.
	wxExit();
}
